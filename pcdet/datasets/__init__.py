...
from .kitti.kitti_semi_dataset import KITTIPretrainDataset, KITTILabeledDataset, KITTIUnlabeledDataset, KITTITestDataset, split_kitti_semi_data, split_kitti_semi_test_data
from .once.once_semi_dataset import ONCEPretrainDataset, ONCELabeledDataset, ONCEUnlabeledDataset, ONCETestDataset, split_once_semi_data, split_once_semi_test_data
...
_semi_dataset_dict = {
    'KittiDataset': {
        ...
        'TEST_FUNC': split_kitti_semi_test_data,
    ...
    'ONCEDataset': {
        ...
        'TEST_FUNC': split_once_semi_test_data,
...
def build_semi_dataloader(dataset_cfg, class_names, batch_size, dist, root_path=None, workers=4,
                     logger=None, merge_all_iters_to_one_epoch=False, save_dir=None):
...
def build_semi_test_dataloader(dataset_cfg, class_names, batch_size, dist, root_path=None, workers=4,
                     logger=None):

    test_infos = _semi_dataset_dict[dataset_cfg.DATASET]['TEST_FUNC'](
        info_paths = dataset_cfg.INFO_PATH,
        data_splits = dataset_cfg.DATA_SPLIT,
        root_path = root_path,
        labeled_ratio = dataset_cfg.LABELED_RATIO,
    ...
    if dist:
        ...
        test_sampler = None
    test_dataloader = DataLoader(
        test_dataset, batch_size=batch_size, pin_memory=True, num_workers=workers,
        shuffle=(test_sampler is None) and False, collate_fn=test_dataset.collate_batch,
        drop_last=False, sampler=test_sampler, timeout=0
    )

    return test_dataset, test_dataloader, test_sampler
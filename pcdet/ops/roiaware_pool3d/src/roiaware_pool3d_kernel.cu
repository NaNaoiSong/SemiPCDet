...
__global__ void num_points_in_boxes_kernel(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *num_points_in_boxes){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z] in LiDAR coordinate
    // params num_points_in_boxes: (B, N), default 0

    int bs_idx = blockIdx.y;
    int box_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (bs_idx >= batch_size || box_idx >= boxes_num) return;

    boxes += bs_idx * boxes_num * 7 + box_idx * 7;
    pts += bs_idx * pts_num * 3;
    num_points_in_boxes += bs_idx * boxes_num + box_idx;

    float local_x = 0, local_y = 0;
    int cur_in_flag = 0;
    for (int k = 0; k < pts_num; k++){
        cur_in_flag = check_pt_in_box3d(pts + k * 3, boxes, local_x, local_y);
        if (cur_in_flag){
            num_points_in_boxes[0] += 1;
        }
    }
}


void num_points_in_boxes_launcher(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, int *num_points_in_boxes){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params num_points_in_boxes: (B, N), default 0
    ...
    dim3 threads(THREADS_PER_BLOCK);
    num_points_in_boxes_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, num_points_in_boxes);

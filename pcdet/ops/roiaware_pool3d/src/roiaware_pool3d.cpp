...
int num_points_in_boxes_gpu(at::Tensor boxes_tensor, at::Tensor pts_tensor, at::Tensor num_points_in_boxes_tensor){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params num_points_in_boxes: (B, N)

//    CHECK_INPUT(boxes_tensor);
//    CHECK_INPUT(pts_tensor);
//    CHECK_INPUT(num_points_in_boxes_tensor);
    ...
    const float *boxes = boxes_tensor.data<float>();
    const float *pts = pts_tensor.data<float>();
    int *num_points_in_boxes = num_points_in_boxes_tensor.data<int>();

    num_points_in_boxes_launcher(batch_size, boxes_num, pts_num, boxes, pts, num_points_in_boxes);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("num_points_in_boxes_gpu", &num_points_in_boxes_gpu, "num_points_in_boxes_gpu forward (CUDA)");
}

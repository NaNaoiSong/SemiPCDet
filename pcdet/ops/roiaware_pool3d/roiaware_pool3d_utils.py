...
def num_points_in_boxes_gpu(points, boxes):
    """
    :param points: (B, M, 3)
    :param boxes: (B, T, 7), num_valid_boxes <= T
    :return num_points_in_boxes: (B, T)
    """
    assert boxes.shape[0] == points.shape[0]
    assert boxes.shape[2] == 7 and points.shape[2] == 3
    batch_size, num_boxes, _ = boxes.shape

    num_points_in_boxes = points.new_zeros((batch_size, num_boxes), dtype=torch.int).fill_(0)
    roiaware_pool3d_cuda.num_points_in_boxes_gpu(boxes.contiguous(), points.contiguous(), num_points_in_boxes)

    return num_points_in_boxes

...
def filter_boxes_method(cls_preds, box_preds, cfgs):
    name = cfgs.NAME
    if name == 'nms':
        ...
        selected = cls_preds > cfgs.SCORE_THRESHOLD

    elif name == 'topk':
        topk = min(cls_preds.shape[0], cfgs.TOPK)
        selected = torch.topk(cls_preds, topk)[1]
    else:
...
def filter_boxes(batch_dict, cfgs):
    ...
    for index in range(batch_size):
        ...
        if not isinstance(batch_dict['batch_cls_preds'], list):
            ...
            if cfgs.get('FILTER_BOX_METHOD', None) is not None:
                selected = filter_boxes_method(max_cls_preds, final_boxes, cfgs.FILTER_BOX_METHOD)
            ...
            num_classes = sum([len(cur_mapping) for cur_mapping in multihead_label_mapping])
            ...
            for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                ...
                cur_max_cls_preds, cur_label_preds = torch.max(cur_cls_preds, dim=-1)
                if cfgs.get('FILTER_BOX_METHOD', None) is not None:
                    selected = filter_boxes_method(cur_max_cls_preds, cur_box_preds, cfgs.FILTER_BOX_METHOD)
                    cur_cls_preds = cur_cls_preds[selected]
                    cur_label_preds = cur_label_preds[selected]
                    cur_box_preds = cur_box_preds[selected]
                cur_all_preds = cur_cls_preds.new_zeros([cur_cls_preds.shape[0], num_classes])
                cur_all_preds[:, cur_cls_idx:cur_cls_idx + len(cur_label_mapping)] = cur_cls_preds

                cur_label_preds = cur_label_mapping[cur_label_preds]
                pred_scores.append(cur_all_preds)
                pred_labels.append(cur_label_preds)
                pred_boxes.append(cur_box_preds)
                cur_start_idx += cur_cls_preds.shape[0]
                cur_cls_idx += len(cur_label_mapping)
            final_cls_preds = torch.cat(pred_scores, dim=0)

    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if trained_epoch % ckpt_save_interval == 0 and rank == 0:
                ...
                stu_ckpt_list = glob.glob(str(ckpt_save_dir / 'student' / 'checkpoint_epoch_*.pth'))
                stu_ckpt_list.sort(key=os.path.getmtime)
                tea_ckpt_list = glob.glob(str(ckpt_save_dir / 'teacher' / 'checkpoint_epoch_*.pth'))
                tea_ckpt_list.sort(key=os.path.getmtime)

                if stu_ckpt_list.__len__() >= max_ckpt_save_num:
                    for cur_file_idx in range(0, len(stu_ckpt_list) - max_ckpt_save_num + 1):
                        os.remove(stu_ckpt_list[cur_file_idx])
                        os.remove(tea_ckpt_list[cur_file_idx])

...
def parse_config():
    ...
    cfg.EXP_GROUP_PATH = args.cfg_file.split('/')[-2]
...
def main():
    ...
    if cfg.get('USE_PRETRAIN_MODEL', False):
        ...
        if len(ckpt_list) > 0:
            ...
            it, start_epoch = pretrain_model.load_params_with_optimizer(
                ckpt_list[-1], to_cpu=dist_train, optimizer=pretrain_optimizer, logger=logger
            )
    ...
    if len(student_ckpt_list) > 0:
        ...
        teacher_ckpt = str(teacher_ckpt_dir / student_ckpt_list[-1].split('/')[-1])
        teacher_model.load_params_from_file(filename=teacher_ckpt, to_cpu=dist_train, logger=logger)
        last_epoch = start_epoch + 1
    else:
